...
/*
Write a PL/SQL program to count the number of students who have enrolled 
Electronics course under the faculty of Shariful Islam. If there are no such students under the above faculty then show a message that "There is no faculty named as Shariful Islam".Otherwise show the total numbers of students to such faculty.
...
DECLARE
       ...
       studentNum student.snum%type := 0;
       courseNum course.cnum%type;
	   courseName course.cname%type := 'Electronics';
	   facultyName faculty.fName%type := 'Shariful Islam';
	   
BEGIN
     select count(distinct snum) into studentNum from enrolled
	 where cnum in (select cnum from course
	 where cname = courseName and Fid in (select Fid from faculty where fName = facultyName));
	 
	 
	 IF studentNum > 0 then
	    DBMS_OUTPUT.PUT_LINE('The number of students is : ' || TO_CHAR(studentNum));
	 ELSE 
	    DBMS_OUTPUT.PUT_LINE('There is no faculty named as Shariful Islam');
	 END IF;
